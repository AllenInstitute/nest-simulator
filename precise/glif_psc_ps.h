...
glif_psc_ps provides five generalized leaky integrate-and-fire
...
glif_psc_ps has a single synapse that is accessible through receptor_port 1.
...
For example, the default parameter setting of glif_psc_ps neuron model was from
...
class glif_psc_ps : public nest::Archiving_Node
{
public:
  glif_psc_ps();

  glif_psc_ps( const glif_psc_ps& );
...
private:
  ...
  friend class nest::UniversalDataLogger< glif_psc_ps >;
  ...
  struct Buffers_
  {
    ...
    nest::UniversalDataLogger< glif_psc_ps > logger_;
  };
  ...
  static nest::RecordablesMap< glif_psc_ps > recordablesMap_;
};


inline size_t
nest::glif_psc_ps::Parameters_::n_receptors_() const
{
  return tau_syn_.size();
}

inline nest::port
nest::glif_psc_ps::send_test_event( nest::Node& target, nest::port receptor_type, nest::synindex, bool )
{
  nest::SpikeEvent e;
  e.set_sender( *this );
...
inline nest::port
nest::glif_psc_ps::handles_test_event( nest::CurrentEvent&, nest::port receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline nest::port
nest::glif_psc_ps::handles_test_event( nest::DataLoggingRequest& dlr, nest::port receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw nest::UnknownReceptorType( receptor_type, get_name() );
...
inline void
glif_psc_ps::get_status( DictionaryDatum& d ) const
{
...
inline void
glif_psc_ps::set_status( const DictionaryDatum& d )
{
