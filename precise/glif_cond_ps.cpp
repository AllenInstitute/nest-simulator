...
template <>
void
DynamicRecordablesMap< nest::glif_cond_ps >::create( glif_cond_ps& host )
{
  insert( names::V_m, host.get_data_access_functor( glif_cond_ps::State_::V_M ) );
  insert( names::I, host.get_data_access_functor( glif_cond_ps::State_::I ) );
  insert( names::ASCurrents_sum, host.get_data_access_functor( glif_cond_ps::State_::ASC_SUM ) );
  insert( names::threshold, host.get_data_access_functor( glif_cond_ps::State_::TH ) );
  insert( names::threshold_spike, host.get_data_access_functor( glif_cond_ps::State_::TH_SPK ) );
  insert( names::threshold_voltage, host.get_data_access_functor( glif_cond_ps::State_::TH_VLT ) );

  host.insert_conductance_recordables();
}

Name
glif_cond_ps::get_g_receptor_name( size_t receptor )
{
...
void
glif_cond_ps::insert_conductance_recordables( size_t first )
{
  for ( size_t receptor = first; receptor < P_.n_receptors_(); ++receptor )
  {
    size_t elem = glif_cond_ps::State_::G_SYN + receptor * glif_cond_ps::State_::NUMBER_OF_STATES_ELEMENTS_PER_RECEPTOR;
    recordablesMap_.insert( get_g_receptor_name( receptor ), this->get_data_access_functor( elem ) );
  }
}

DataAccessFunctor< glif_cond_ps >
glif_cond_ps::get_data_access_functor( size_t elem )
{
  return DataAccessFunctor< glif_cond_ps >( *this, elem );
}
}
 ...
 * ---------------------------------------------------------------- */
extern "C" inline int
nest::glif_cond_ps_dynamics( double, const double y[], double f[], void* pnode )
{
  // a shorthand
  typedef nest::glif_cond_ps::State_ S;

  // get access to node so we can almost work as in a member function
  assert( pnode );
  const nest::glif_cond_ps& node = *( reinterpret_cast< nest::glif_cond_ps* >( pnode ) );
...
nest::glif_cond_ps::Parameters_::Parameters_()
  : G_( 9.43 )               // in nS
  , E_L_( -78.85 )           // in mV
...
nest::glif_cond_ps::State_::State_( const Parameters_& p )
...
nest::glif_cond_ps::State_::State_( const State_& s )
{
...
nest::glif_cond_ps::State_& nest::glif_cond_ps::State_::operator=( const State_& s )
{
...
void
nest::glif_cond_ps::Parameters_::get( DictionaryDatum& d ) const
{
...
double
nest::glif_cond_ps::Parameters_::set( const DictionaryDatum& d )
{
...
void
nest::glif_cond_ps::State_::get( DictionaryDatum& d, const Parameters_& p ) const
{
...
void
nest::glif_cond_ps::State_::set( const DictionaryDatum& d, const Parameters_& p, double delta_EL )
{
...
nest::glif_cond_ps::Buffers_::Buffers_( glif_cond_ps& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::glif_cond_ps::Buffers_::Buffers_( const Buffers_& b, glif_cond_ps& n )
  : logger_( n )
  , s_( 0 )
  , c_( 0 )
  , e_( 0 )
...
nest::glif_cond_ps::glif_cond_ps()
  : Archiving_Node()
  , P_()
  , S_( P_ )
  , B_( *this )
...
nest::glif_cond_ps::glif_cond_ps( const glif_cond_ps& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
void
nest::glif_cond_ps::init_state_( const Node& proto )
{
  const glif_cond_ps& pr = downcast< glif_cond_ps >( proto );
  S_ = pr.S_;
}

void
nest::glif_cond_ps::init_buffers_()
{
  ...
  B_.sys_.function = glif_cond_ps_dynamics;
...
void
nest::glif_cond_ps::update( Time const& origin, const long from, const long to )
{
  // initial values
...
nest::port
nest::glif_cond_ps::handles_test_event( SpikeEvent&, rport receptor_type )
{
...
void
nest::glif_cond_ps::handle( SpikeEvent& e )
{
...
void
nest::glif_cond_ps::handle( CurrentEvent& e )
{
  assert( e.get_delay_steps() > 0 );
...
void
nest::glif_cond_ps::handle( DataLoggingRequest& e )
{
