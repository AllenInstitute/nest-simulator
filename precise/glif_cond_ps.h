...
/* BeginDocumentation
Name: glif_cond_ps - Conductance-based generalized leaky integrate and fire (GLIF)
model

Description:

glif_cond_ps provides five generalized leaky integrate
...
For example, the default parameter setting of this glif_cond_ps neuron model was from
...
extern "C" int glif_cond_ps_dynamics( double, const double*, double*, void* );

class glif_cond_ps : public nest::Archiving_Node
{
public:
  glif_cond_ps();

  glif_cond_ps( const glif_cond_ps& );
...
private:
  ...
  friend int glif_cond_ps_dynamics( double, const double*, double*, void* );

  // The next three classes need to be friends to access the State_ class/member
  friend class DynamicRecordablesMap< glif_cond_ps >;
  friend class DynamicUniversalDataLogger< glif_cond_ps >;
  friend class DataAccessFunctor< glif_cond_ps >;
  ...
  struct Buffers_
  {
    Buffers_( glif_cond_ps& );
    Buffers_( const Buffers_&, glif_cond_ps& );
    ...
    DynamicUniversalDataLogger< glif_cond_ps > logger_;
  ...
  DynamicRecordablesMap< glif_cond_ps > recordablesMap_;

  // Data Access Functor getter
  DataAccessFunctor< glif_cond_ps > get_data_access_functor( size_t elem );
  inline double
  get_state_element( size_t elem )
  {
    if ( elem == nest::glif_cond_ps::State_::V_M )
    {
      return S_.y_[ elem ] + P_.E_L_;
    }
    else if ( elem == nest::glif_cond_ps::State_::I )
    {
      return B_.I_;
    }
    else if ( elem == nest::glif_cond_ps::State_::ASC_SUM )
    {
      return S_.ASCurrents_sum_;
    }
    else if ( elem == nest::glif_cond_ps::State_::TH )
    {
      return S_.threshold_ + P_.E_L_;
    }
    else if ( elem == nest::glif_cond_ps::State_::TH_SPK )
    {
      return S_.threshold_spike_;
    }
    else if ( elem == nest::glif_cond_ps::State_::TH_VLT )
    {
      return S_.threshold_voltage_;
    }
    else
    {
      return S_.y_[ elem - nest::glif_cond_ps::State_::NUMBER_OF_RECORDABLES_ELEMENTS ];
    }
  };
...
inline size_t
nest::glif_cond_ps::Parameters_::n_receptors_() const
{
...
inline nest::port
nest::glif_cond_ps::send_test_event( nest::Node& target, nest::port receptor_type, nest::synindex, bool )
{
  nest::SpikeEvent e;
  e.set_sender( *this );
...
inline nest::port
nest::glif_cond_ps::handles_test_event( nest::CurrentEvent&, nest::port receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline nest::port
nest::glif_cond_ps::handles_test_event( nest::DataLoggingRequest& dlr, nest::port receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw nest::UnknownReceptorType( receptor_type, get_name() );
...
inline void
glif_cond_ps::get_status( DictionaryDatum& d ) const
{
...
inline void
glif_cond_ps::set_status( const DictionaryDatum& d )
{
